...
class Player(Base):
    ...
    # 每用户世界时钟:world_accum 为累计世界秒(在线 1× / 离线 offline_factor)
    ...
    # 每用户世界速率覆盖:None=用全局 time_scale;非 None=覆盖全局(管理端可单独调速)
    time_scale_override: Mapped[float | None] = mapped_column(Float, nullable=True)
