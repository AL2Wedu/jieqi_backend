...
- **头衔 / 成就**:`titles` / `achievements` 表已建,但无获得、解锁、佩戴逻辑(已定:成就解锁头衔)
- **学习进度**:`term_progress` / `user_quiz_records` 表已建,解锁逻辑未实现
- **教育内容**:知识卡片(`knowledge_items`)/ 每日一题(`quiz_questions`)无数据、无接口
- **AI 链路**:`ai_sessions` / `ai_messages`(审核留痕)表已建,LLM 接入 / pgvector RAG 未开始
- **社交**:好友、班级、排行榜均未做
- **任务 / 活动**:`quests` / `user_quests` 未做
- **埋点分析**:`event_logs` / `play_sessions` 未采集
- **数据库迁移链**:Alembic 未初始化(开发用 `create_all` 建表)
- **正式部署落地**:PostgreSQL 实机、Nginx、systemd 未实际配置(部署指南已写好,见第 9 节)

## 1.4 计划中 📋(路线图)

### P1(下一步)
- 教育内容模块:节气知识卡片 + 每日一题 + "学完本节气 → 解锁下一节气"教学闭环
- AI 问答:节气小老师 NPC,pgvector 检索知识库,输出过审 + 全链路留痕(未成年人合规)
- 排行榜;Redis pub/sub 改造节气广播;Alembic 迁移链
